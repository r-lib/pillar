...
How to customize the printed output of a `"tbl"` subclass? This vignette
shows the various customization options. Customizing the formatting of a
vector class in a tibble is described in
[`vignette("pillar", package = "vctrs")`](https://vctrs.r-lib.org/articles/pillar.html).
An overview over the control and data flow is given in
[`vignette("printing")`](https://pillar.r-lib.org/articles/printing.md).

This vignette assumes that the reader is familiar with S3 classes,
methods, and inheritance. The [“S3”
chapter](https://adv-r.hadley.nz/s3.html) of Hadley Wickham’s “Advanced
R” is a good start.

To make use of pillar’s printing capabilities, create a class that
inherits from `"tbl"`, like tibble (classes `"tbl_df"` and `"tbl"`),
dbplyr lazy tables (`"tbl_lazy"` and `"tbl"`) and sf spatial data frames
(`"sf"`, `"tbl_df"` and `"tbl"`). Because we are presenting various
customization options, we create a constructor for an example data frame
...
The `"default"` class doesn’t have any customizations yet, and prints
...
The easiest customization consists of tweaking the header. Implement a
[`tbl_sum()`](https://pillar.r-lib.org/reference/tbl_sum.md) method to
extend or replace the information shown in the header, keeping the
original formatting.
...
method. Here, as in all `tbl_format_*()` methods, you can use the
information contained in the setup object, see
[`?new_tbl_format_setup`](https://pillar.r-lib.org/reference/new_tbl_format_setup.md)
...
If the same information needs to be displayed in several parts (e.g., in
both header and footer), it is useful to compute it in
[`tbl_format_setup()`](https://pillar.r-lib.org/reference/tbl_format_setup.md)
and store it in the setup object. New elements may be added to the setup
object, existing elements should not be overwritten. Exception: the
`tbl_sum` element contains the output of
[`tbl_sum()`](https://pillar.r-lib.org/reference/tbl_sum.md) and can be
enhanced with additional elements.
...
instead of Arabic numerals, one could use
[`utils::as.roman()`](https://rdrr.io/r/utils/roman.html) to generate
the corresponding sequence and build up a row ID pillar using
[`new_pillar()`](https://pillar.r-lib.org/reference/new_pillar.md) and
associated methods as has been introduced previously.
...
To make the width adaptive, we implement a `"rule"` class with a
[`format()`](https://rdrr.io/r/base/format.html) method that formats
...
Compound pillars are created by
...
implementation also calls
[`ctl_new_pillar()`](https://pillar.r-lib.org/reference/ctl_new_pillar.md)
shown above. The (somewhat artificial) example hides all data frame
...
Last but not least, it is also possible to completely alter the display
of the body by overriding
...
[`tbl_format_setup()`](https://pillar.r-lib.org/reference/tbl_format_setup.md),
this takes a considerable amount of time. If you really need to change
the output for the entire body, consider providing your own
[`tbl_format_setup()`](https://pillar.r-lib.org/reference/tbl_format_setup.md)
method.
