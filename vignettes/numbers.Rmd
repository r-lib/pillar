...
tibble(
  x4 = num(8:12 * 100 + 0.5, 4),
  x1 = num(8:12 * 100 + 0.5, digits = 1),
  usd = num(8:12 * 100 + 0.5, digits_fixed = 2, label = "USD"),
...
# Need to avoid unclass() in units: https://github.com/r-quantities/units/pull/275
set_units(num(1:3, notation = "sci"), m) + set_units(1:3, km)
...
# FIXME: subsetting needs to preserve pillar attribute
...
pillar_shaft.formattable_currency <- function(x, ...) {
  ...
  pillar_shaft(num(unclass(x), digits_fixed = formattable$digits))
}

pillar_shaft.formattable_percent <- function(x, ...) {
  formattable <- attr(x, "formattable")

  pillar_shaft(num(unclass(x), digits_fixed = formattable$digits, scale = 100))
}
