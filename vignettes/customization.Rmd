...
If the components of a pillar are components with multiple cells, the pillar can be decomposed into sub-pillars that consist of corresponding cells from each component box.
...
- Rethink `new_pillar_box()` and `pillar_box()`, closer coupling with `new_ornament()`?
    - Boxes and ornaments are different concepts: a box is used to describe the components of a pillar, an ornament has the contents of a shaft (or of another component)
    - Start with documenting `new_pillar_box()` etc.
