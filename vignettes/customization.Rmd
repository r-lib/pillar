...
How to format? Do we need to unpack for printing? Is this a decision made by the container, or when printing?

Alternative: show only first column, with dots at end. (Never discussed, this is not how data frames print.)

Current situation: names become too wide and are hard to parse, especially when wrapping. Multistage names?

Unclear: is the name part of the pillar? Perhaps yes, then for packed data frames and matrix columns we need a vectorized object that knows how to split itself.

Design of new generics should be flexible enough to allow for future changes.
...
Two new generics, `ctl_new_pillar()` and `ctl_new_single_pillar()`, dispatch on that controller.
...
ctl_new_pillar.tbl <- function(controller, x, width, ..., title = NULL) {
  if (is.data.frame(x)) {
    ...
    ctl_new_single_pillar(controller, x, width, ..., title = title)
  }
}

ctl_new_single_pillar.tbl <- function(controller, x, width, ..., title = NULL) {
...
```{r}
ctl_new_single_pillar.foo_controller <- function(controller, x, width, ..., title = NULL) {
  out <- NextMethod()
...
- The call `new_data_frame_pillar(as.data.frame(x), x, width, title = NULL)` gives us the representation of the entire data frame. A similar call is used for packed data frames.
