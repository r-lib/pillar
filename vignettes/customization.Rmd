...
Each component is a `"pillar_component"`, as described below.
...
### The `"pillar_component"` class

An object of the `"pillar_component"` class captures contents that can be fitted in a rectangle.
Each component consists of one or multiple cells that are aligned horizontally (with one space inbetween) when printed.
...
The cells may be stored as a list with one element per cell (for `type` and `data` components), or as a hierarchy of components (for the `name` component).
In the latter case, `length(x) < length(get_cell_widths(x))`, and each element must be a component.
...
get_cells <- function(x, from, to) {
  ...
  if (length(x) < length(get_cell_widths(x))) {
    ...
    idx <- seq2(from, to)
    new_pillar_component(x[idx], get_cell_widths(x)[idx], get_cell_min_widths(x)[idx])
  }
}
...
If the components of a pillar are components with multiple cells, the pillar can be decomposed into sub-pillars that consist of corresponding cells from each component component.
For each sub-pillar, its width can be computed as a maximum of the cell widths of the individual components.
...
- Document control and data flow in a new vignette

    - What operations happen when? Are we overspending time recomputing e.g. alignment or calling `format()`? Do we really need to do S3 dispatch for every cell?
    - Needs better printing for individual steps
    - Why does printing of compound pillars even work? Transpose? Do we need this?
    - `ctl_colonnade()` is opaque, for good reason?

- Document all functions
- Rethink `new_pillar_component()` and `pillar_component()`, closer coupling with `new_ornament()`?
    - Components and ornaments are different concepts: a component is used to describe the components of a pillar, an ornament has the contents of a shaft (or of another component)
    - Start with documenting `new_pillar_component()` etc.
