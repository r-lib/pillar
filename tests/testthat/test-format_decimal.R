...
test_that("output test", {
  expect_pillar_output(xp = add_special((10^(-3:4)) * c(-1, 1)), filename = "basic.txt")
  expect_pillar_output(xp = add_special(1.23456 * 10^(-3:3)), filename = "decimal-insignif.txt")
  withr::with_options(
    list(pillar.sigfig = 5),
    expect_pillar_output(xp = add_special((10^(c(-2:4, 0L))) * c(-1, 1)), filename = "basic-signif-5.txt")
  )
  withr::with_options(
    list(pillar.sigfig = 7),
    expect_pillar_output(xp = add_special((10^(0:-5)) * c(-1, 1)), width = 20, filename = "basic-signif-7.txt")
  )
  expect_pillar_output(xp = add_special((10^(5:-5)) + 1e-7), width = 20, filename = "basic-slightly-nonint.txt")
})
