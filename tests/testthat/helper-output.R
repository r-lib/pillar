...
get_pillar_output_object <- function(x = NULL, xp = NULL, xf = NULL, ...) {
  #' @details
  #' - If `x` is passed, `xp` is computed by appending `NA` (and for numerics
  #'   `+Inf` and `-Inf`).
  xp <- xp %||% add_special(x)

  #' - If `xp` is passed or available computed from `x`, `xf` is derived by
  #'   calling `pillar(xp, ...)`.
  xf <- xf %||% pillar(xp, ...)

  #' - The output is generated by printing `xf`.
  xf
}
