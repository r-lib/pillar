test_that("integer64 output will use scientific if necessary", {
  ...
  expect_snapshot({pillar(add_special(x), width = 10)})
  expect_snapshot({pillar(add_special(x), width = 11)})
  expect_snapshot({pillar(add_special(x), width = 12)})
  expect_snapshot({pillar(add_special(x), width = 13)})
  expect_snapshot({pillar(add_special(x), width = 14)})
})
