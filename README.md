...
[![CRAN
status](http://www.r-pkg.org/badges/version/pillar)](https://cran.r-project.org/package=pillar)
...
pillar is not designed for end-users but will eventually be incorporated
in packages like [tibble](http://tibble.tidyverse.org).
...
[tibble](https://github.com/tidyverse/tibble), which in the current
development version already lets pillar do all the formatting work.
Packages that implement a data type to be used in a tibble column can
add color with only a few changes:
...
Replace `"your_class_name"` with the name of the S3 class of your data
type.
